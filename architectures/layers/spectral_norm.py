...
def spectral_norm_conv2d(incoming, filters, kernel_size=3, stride=1, padding='same', iteration=1,
                         trainable=True, scope='sn_conv'):
    """
    wrapper to SpectralNormConv2D class
    """
    conv_norm = SpectralNormConv2D(filters, kernel_size, stride, padding, iteration, trainable, scope)
    return conv_norm.call(incoming)


class SpectralNormConv2D(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size=3, stride=1, padding='same', iteration=1, trainable=True, scope='sn_conv'):
        """
    ...
    def call(self, inputs, **kwargs):
        ...
        with tf.variable_scope(self.scope):
            w = tf.get_variable("kernel",
                                shape=[self.kernel_size, self.kernel_size, inputs.get_shape()[-1], self.filters],
