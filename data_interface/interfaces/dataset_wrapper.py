...
class DatasetInterfaceWrapper(object):
    ...
    def get_acdc_sup_data(self, data_path, data_ids, repeat=False, seed=None):
        ...
        itf = ACDCSupInterface(root_dir=data_path, data_ids=data_ids, input_size=self.input_size,
                                    verbose=self.verbose)

        train_init, valid_init, test_init, input_data, output_data = itf.get_data(
    ...
    def get_acdc_disc_data(self, data_path, data_ids, repeat=False, seed=None, b_size=None):
        """
        wrapper to ACDC data set. Gets input images and annotated masks.
        :param data_path: (str) path to data directory
        :param data_ids: (dict) dictionary with train, validation, test volume ids
        ...
        # initialize data set interfaces
        itf = ACDCDiscInterface(root_dir=data_path, data_ids=data_ids, input_size=self.input_size,
                                     verbose=self.verbose)

        train_init, valid_init, input_data = itf.get_data(
            b_size=b_size,
            augment=self.augment,
            repeat=repeat,
            num_threads=self.num_threads,
    ...
    def get_acdc_unsup_data(self, data_path, data_ids, repeat=False, seed=None):
        ...
        # initialize data set interfaces
        itf = ACDCUnsupInterface(root_dir=data_path, data_ids=data_ids, input_size=self.input_size,
                                      verbose=self.verbose)
